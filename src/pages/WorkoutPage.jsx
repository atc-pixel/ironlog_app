...
import { Activity, Plus, Check, Save, Trash2, History, ArrowLeft, ChevronRight, X } from "../components/Icons";
import RestTimerOverlay from "../components/RestTimerOverlay";
import ExerciseSelector from "../components/ExerciseSelector";
import WorkoutFinishScreen from "../components/WorkoutFinishScreen";

// --- YENİ MODAL: HAREKET EKLEME ---
const AddExerciseModal = ({ value, onChange, onSave, onCancel }) => (
  <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-6 animate-in fade-in duration-200">
      <div className="bg-slate-900 w-full max-w-sm p-6 rounded-3xl border border-slate-800 shadow-2xl animate-in zoom-in-95 duration-200">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-blue-500 font-bold uppercase tracking-widest text-xs">Yeni Hareket Oluştur</h3>
            <button onClick={onCancel} className="text-slate-500 hover:text-white"><X size={20} /></button>
          </div>
          <input autoFocus type="text" value={value} onChange={onChange} placeholder="Örn: Front Squat" className="w-full bg-slate-950 text-white font-bold text-xl p-4 rounded-2xl border border-slate-800 focus:border-blue-500 outline-none mb-6 placeholder:text-slate-700 transition-colors" />
          <div className="flex gap-3">
              <button onClick={onCancel} className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-400 py-3.5 rounded-xl font-bold text-sm transition-colors">İptal</button>
              <button onClick={onSave} className="flex-1 bg-blue-600 hover:bg-blue-500 text-white py-3.5 rounded-xl font-bold text-sm transition-colors">Kaydet</button>
          </div>
      </div>
  </div>
);
...
export default function WorkoutPage({ onFinishAndExit, initialDate }) {
  ...
  if (isFinished) {
    return <WorkoutFinishScreen date={initialDate} totalSets={totalSets} totalVolume={totalVolume} onSave={saveAndExit} />;
  }

  return (
    // GÜNCELLEME: Flex Column yapısı ile ekranı katı bloklara bölüyoruz.
    <div className="h-[100dvh] bg-slate-950 text-slate-100 font-sans flex flex-col overflow-hidden">
      
      {/* --- KATMAN 1: MODALLAR (En Üst) --- */}
      ...
      <ExerciseSelector isOpen={isSelectorOpen} onClose={() => setIsSelectorOpen(false)} onSelect={handleExerciseSelect} customExercises={customExercises} />
      {isAddingNewExercise && <AddExerciseModal value={newExerciseName} onChange={(e) => setNewExerciseName(e.target.value)} onSave={saveNewExercise} onCancel={cancelNewExercise} />}

      {/* --- KATMAN 2: SABİT ÜST BLOK (Asla Kaymaz) --- */}
      <div className="flex-none z-30 bg-slate-950 border-b border-slate-900 shadow-xl">
        
        {/* HEADER */}
        <header className="px-4 pt-14 pb-2 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button onClick={onFinishAndExit} className="bg-slate-900 p-2.5 rounded-xl border border-slate-800 text-slate-400 hover:text-white active:scale-95 transition-all">
              <ArrowLeft size={20} />
            </button>
            <div>
          ...
          <div className="bg-slate-900 p-2 rounded-xl border border-slate-800"><Activity size={20} className="text-blue-500" /></div>
        </header>

        {/* YENİ: HAREKET KONTROL BARI (Yenilenmiş Tasarım) */}
        <div className="px-4 pb-4 pt-2">
           <div className="w-full bg-slate-900 rounded-2xl border border-slate-800 p-1.5 flex items-center justify-between shadow-inner relative overflow-hidden">
              {/* Arkaplan Efekti */}
              <div className="absolute inset-0 bg-gradient-to-r from-blue-500/5 to-transparent pointer-events-none"></div>
              
              {/* Sol Ok (Önceki Hareket - Şimdilik görsel) */}
              <button className="p-3 rounded-xl text-slate-500 hover:bg-slate-800 hover:text-white transition-colors">
                 <ChevronRight size={18} className="rotate-180" />
              </button>

              {/* Orta: Hareket İsmi (Tıklanabilir) */}
              <button onClick={() => setIsSelectorOpen(true)} className="flex-1 flex flex-col items-center justify-center py-1 group active:scale-95 transition-transform">
                 <span className="text-[10px] text-blue-500 font-bold uppercase tracking-widest mb-0.5">Aktif Hareket</span>
                 <div className="flex items-center gap-2">
                    <span className="text-white font-black text-lg tracking-tight truncate max-w-[200px]">{selectedExercise}</span>
                    <div className="text-slate-600 group-hover:text-white transition-colors"><ChevronRight size={12} className="rotate-90" strokeWidth={4} /></div>
                 </div>
              </button>

              {/* Sağ Ok (Sonraki Hareket - Şimdilik görsel) */}
              <button className="p-3 rounded-xl text-slate-500 hover:bg-slate-800 hover:text-white transition-colors">
      ...
      {/* --- KATMAN 3: KAYAN İÇERİK (Sadece burası kayar) --- */}
      <div className="flex-1 overflow-y-auto overflow-x-hidden pb-safe px-4 space-y-6 scrollbar-hide pt-4">
         {!isAddingNewExercise && (
            <>
              {/* Panel: Ağırlık & Tekrar */}
