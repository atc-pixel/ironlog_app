...
export default function WorkoutPage({ onFinishAndExit, initialDate }) {
  ...
  // --- FONKSİYONLAR (Eski kodun aynısı, değişiklik yok) ---
  ...
  if (isFinished) {
    return (
      <WorkoutFinishScreen date={initialDate} totalSets={totalSets} totalVolume={totalVolume} onSave={saveAndExit} />
    );
  }
  ...
  return (
    <div className="h-[100dvh] bg-slate-950 text-slate-100 font-sans flex flex-col overflow-hidden">
      ...
      {/* GÜNCELLEME: pt-6 yerine pt-14 */}
      <header className="flex-none px-4 pt-14 pb-2 flex items-center justify-between bg-slate-950 z-30">
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden pb-safe px-4 space-y-6 scrollbar-hide">
         <div className="mt-2">
            {!isAddingNewExercise ? (
              <button onClick={() => setIsSelectorOpen(true)} className="w-full bg-slate-900 text-white font-bold text-xl py-4 px-6 rounded-2xl border border-slate-800 flex items-center justify-between shadow-lg transition-all active:scale-[0.98]">
