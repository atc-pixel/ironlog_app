...
export default function WorkoutPage({ onFinishAndExit, initialDate }) {
  ...
  // Veri Aralıkları
  const weightOptions = Array.from({ length: 41 }, (_, i) => i * 5); // 0-200kg arası
  const repOptions = Array.from({ length: 28 }, (_, i) => i + 3); // 3-30 tekrar

  // Hareket Seçimi Değişince
  ...
  // Geçmiş Verileri Bul (Progressive Overload için)
  const exerciseHistory = history
    ? history.map(day => {
        const found = day.exercises.find(e => e.name === selectedExercise);
        if (day.date === initialDate) return null; // Bugünü gösterme
  ...
  const addSet = () => {
    ...
    // Ayarlar açıksa sayacı başlat
    if (settings.restTimerEnabled) {
      setIsTimerActive(true);
    }
  };

  // Hareketi Bitirme ve Listeye Ekleme/Birleştirme
  const finishExercise = () => {
    ...
    setWorkoutLog(prevLog => {
      ...
      if (existingIndex !== -1) {
        // Varsa üstüne ekle (Merge)
        const updatedLog = [...prevLog];
        const existingExercise = updatedLog[existingIndex];
        updatedLog[existingIndex] = {
          ...existingExercise,
          sets: [...existingExercise.sets, ...currentSets]
        };
        return updatedLog;
      } else {
        // Yoksa yeni oluştur
        const completedExercise = { id: Date.now(), name: selectedExercise, sets: currentSets };
        return [...prevLog, completedExercise];
      }
    });
  ...
  const removeCompletedExercise = (exerciseId) => {
    setWorkoutLog(workoutLog.filter(ex => ex.id !== exerciseId));
  };
  ...
  if (isFinished) {
    return (
      <div className="h-[100dvh] bg-slate-950 flex flex-col items-center justify-center p-6 animate-in zoom-in duration-300">
         ...
         <button onClick={saveAndExit} className="w-full bg-green-600 hover:bg-green-500 text-white font-bold py-4 rounded-2xl text-xl shadow-lg shadow-green-900/50">
            Kaydet ve Çık
         </button>
      </div>
    );
    ...
    <div className="h-[100dvh] bg-slate-950 text-slate-100 font-sans flex flex-col overflow-hidden">
      ...
      <header className="flex-none px-4 pt-6 pb-2 flex items-center justify-between bg-slate-950 z-30">
        <div className="flex items-center gap-4">
          <button onClick={handleBackWithoutSaving} className="bg-slate-900/80 p-3 rounded-xl border border-slate-800 text-slate-400 hover:text-white active:scale-95 transition-all">
            <ArrowLeft size={20} />
          </button>
          <div>
            <h1 className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-500 italic tracking-tighter antialiased leading-none">
              IRONLOG
            </h1>
            ...
            {!isAddingNewExercise ? (
              <div className="relative">
                <select 
                  ...
                  className="w-full bg-slate-900 text-white font-bold text-xl py-4 px-6 rounded-2xl border border-slate-800 focus:border-blue-500 outline-none appearance-none shadow-lg transition-all antialiased"
                >
              ...
              <div className="bg-slate-900 p-4 rounded-2xl border border-blue-500/50 shadow-lg animate-in fade-in slide-in-from-top-2">
                 ...
                 <input 
                    ...
                    placeholder="Örn: Triceps Pushdown"
                    className="w-full bg-slate-950 text-white font-bold text-lg p-3 rounded-xl border border-slate-700 focus:border-blue-500 outline-none mb-3"
                 />
                 <div className="flex gap-3">
